...
class RNN(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        h = torch.zeros(batch_size, lstm_hidden_state_size, device=device)
        c = torch.zeros(batch_size, lstm_hidden_state_size, device=device)
...
def train(model: torch.nn.Module, train_dataloader: DataLoader, dev_dataloader: DataLoader):
    ...
    accuracy = BinaryAccuracy(threshold=accuracy_threshold).to(device)
    ...
    for epoch in range(epochs):
        ...
        for i, (inputs, labels) in enumerate(train_dataloader):
            ...
            total_train_accuracy.append(accuracy(output, labels).cpu())
        ...
        with torch.no_grad():
            ...
            for dev_inputs, dev_labels in dev_dataloader:
                ...
                current_accuracy = accuracy(dev_output, dev_labels).cpu()
