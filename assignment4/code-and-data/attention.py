...
def create_kqv_matrix(input_vector_dim, n_heads = 1):
    assert input_vector_dim % n_heads == 0, "input_vector_dim must be divisible by n_heads"
    # Output 3 projections: key, query, value
    output_dim = 3 * input_vector_dim  # or 3 * head_dim * n_heads
    return nn.Linear(input_vector_dim, output_dim)

def perform_kqv(x, linear):
    B, N, D = x.size()
    kqv_matrix = linear(x)
    k,q,v = kqv_matrix.reshape(B,N,3,D).unbind(dim=2)
    return k, q, v
